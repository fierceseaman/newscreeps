    ...
    getPath: function(creep, goals, avoidEnemies, maxRooms, boundingBox){
        ...
        const result = PathFinder.search(creep.pos, goals, {
            ...
            roomCallback: function(roomName){
                ...
                if(roomName != creep.pos.roomName && roomData.own && !settings_1.allies.includes(roomData.own) 
    ...
    getRoute: function(start, finish, avoidEnemies){
        ...
        const route = Game.map.findRoute(start, finish, {
            routeCallback: function(roomName){
                ...
                const roomData = utils.getsetd(roomDataCache, roomName, {});
                if(roomData.own && !settings_1.allies.includes(roomData.own) && roomData.rcl && CONTROLLER_STRUCTURES[STRUCTURE_TOWER][roomData.rcl] && avoidEnemies){
                    return 5
                }
        ...
        return _(cities)
            ...
            .map((neighbors, city) => {
                return _(neighbors).filter(neighbor => {
                    ...
                    return !settings_1.allies.includes(data.own) && data.rcl <= 6 
    ...
    runBorderTug: function(creep, pullee, destination){
        ...
        const nextRoomDir = Game.map.findExit(creep.pos.roomName, endRoom, {
            routeCallback: function(roomName){
                ...
                if(roomData.own && !settings_1.allies.includes(roomData.own)) return 50
                if(Memory.remotes[roomName]) return 1
                return 50
            }
        });
...
var rr = {
    ...
    getRolePriorities: function(){
        ...
        priorites[defender.name] = 2;
        priorites[transporter.name] = 1;
            ...
            for(const room of myCities){
                ...
                const result = PathFinder.search(room.controller.pos, {pos: controllerPos, range: 1}, {
                    plainCost: 1, swampCost: 1, maxOps: 10000, roomCallback: (roomName) => {
                        if(!Cache.roomData[roomName] || (Cache.roomData[roomName].rcl && CONTROLLER_STRUCTURES[STRUCTURE_TOWER][Cache.roomData[roomName].rcl] && !settings_1.allies.includes(Cache.roomData[roomName].own)))
                            return false
                    }
                });
...
commonjsGlobal.RemoveJunk = function(city){//only to be used on cities with levelled factories
